...
import { desktopApi, openDirectory, selectEngineDirectory, selectEngineExecutable } from "../../services/desktop";
...
export function SettingsDrawer({ engines, settings, metrics, projectPath, onClose, onSettings, onEngines, onError }: {
  engines: EngineInfo[];
  ...
  return (
    <aside className="settings-drawer" aria-label="应用设置">
      <header><div><Gear size={20} /><div><strong>设置与系统状态</strong><span>更改将在下一次阶段启动时生效</span></div></div><button className="icon-button" type="button" onClick={onClose} aria-label="关闭设置"><X size={18} /></button></header>
      <nav>{(["engines", "defaults", "performance", "diagnostics"] as SettingsTab[]).map((value) => <button type="button" key={value} className={tab === value ? "is-active" : ""} onClick={() => setTab(value)}>{value === "engines" ? "引擎" : value === "defaults" ? "项目默认值" : value === "performance" ? "性能" : "诊断"}</button>)}</nav>
      <div className="settings-content">
        {tab === "engines" && <section className="settings-section"><div className="settings-section-title"><div><h2>引擎定位</h2><p>环境变量优先于应用设置，Pipeline 与预检使用同一结果。</p></div><button className="button button-secondary" type="button" disabled={busy} onClick={refreshEngines}>重新检测</button></div>
          ...
          <div className="engine-settings-list">{engines.map((engine) => <article key={engine.name}><div className="engine-card-title"><strong>{engine.name}</strong><span className={engine.available ? "analysis-ok" : "analysis-blocked"}>{engine.available ? "已验证" : "未找到"}</span></div><dl><div><dt>版本</dt><dd>{engine.version ?? "尚未读取"}</dd></div><div><dt>来源</dt><dd>{engine.source === "environment" ? "METORIGIN_ENGINE_DIR" : engine.source === "resource" ? "应用资源" : engine.source === "configured_or_path" ? "自定义 / PATH" : "未定位"}</dd></div><div><dt>路径</dt><dd title={engine.path ?? undefined}>{engine.path ?? "尚未定位"}</dd></div></dl><div className="engine-card-actions"><button type="button" onClick={() => void run(async () => { const path = await selectEngineExecutable(engine.name); if (path) { onSettings(await desktopApi.setEngineExecutable(engine.name, path)); onEngines(await desktopApi.checkEngines()); } })}>单独定位</button><button type="button" disabled={!engine.path} onClick={() => engine.path && void openDirectory(engine.path)}>打开位置</button></div></article>)}</div>
        ...
        {tab === "diagnostics" && <section className="settings-section"><div className="settings-section-title"><div><h2>脱敏诊断包</h2><p>包含项目状态、日志尾部、引擎和硬件摘要；不包含视频、图片或 PLY。</p></div></div><button className="button button-primary" type="button" disabled={!projectPath || busy} onClick={() => projectPath && void run(async () => { const result = await desktopApi.exportDiagnostics(projectPath); await openDirectory(result.path); })}><DownloadSimple size={16} />导出诊断包</button>{!projectPath && <div className="settings-note">请先打开一个项目。</div>}</section>}
      </div>
    </aside>
  );
}
