...
impl ColmapMapper {
    ...
    pub fn analyze_result(sparse_dir: &Path) -> AppResult<ColmapResult> {
...
fn analyze_model(model_dir: &Path) -> AppResult<ModelInfo> {
    ...
    Ok(ModelInfo {
        cameras: parse_int_field(&stdout, "Cameras:"),
        images: parse_int_field(&stdout, "Images:"),
        registered_images: parse_int_field(&stdout, "Registered images:"),
        point_count: parse_int_field(&stdout, "Points:"),
        observations: parse_int_field(&stdout, "Observations:"),
        mean_track_length: parse_float_field(&stdout, "Mean track length:"),
        mean_reprojection_error: parse_float_field(&stdout, "Mean reprojection error:"),
    })
}
